...
def clean_demand_per_group(demand: pd.DataFrame) -> pd.DataFrame:
    """Cleans the demand data for each unique (supermarket, SKU) pair by filling in missing values.
    
    The function is iterating over unique combinations of (supermarket, SKU) pairs in the provided 
    'demand' DataFrame. For each of such combination, it applies the 'clean' function to the 'demand' column.
    ...
    ----------
    pd.DataFrame
        The cleand input DataFrame with filled missing values with the "demand" column
            cleaned for each supermarket and SKU combination.
    """
    sus = demand.supermarket.unique()
    skus = demand.sku.unique()
    for su in sus:
        for sku in skus:
            demand.loc[(demand.sku == sku) & (demand.supermarket == su), "demand"] = clean(
                demand.loc[(demand.sku == sku) & (demand.supermarket == su), "demand"])
    return demand
